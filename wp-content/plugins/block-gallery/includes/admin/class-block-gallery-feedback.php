...
/**
 * Main Block Gallery Feedback Notice Class
 */
class Block_Gallery_Feedback {
	...
	public function seconds_to_words( $seconds ) {
		...
		if ( $years > 1 ) {
			...
			return sprintf( __( '%s years', 'block-gallery' ), $years );
		} elseif ( $years > 0 ) {
			return __( 'a year', 'block-gallery' );
		}
		...
		if ( $weeks > 1 ) {
			...
			return sprintf( __( '%s weeks', 'block-gallery' ), $weeks );
		} elseif ( $weeks > 0 ) {
			return __( 'a week', 'block-gallery' );
		}
		...
		if ( $days > 1 ) {
			...
			return sprintf( __( '%s days', 'block-gallery' ), $days );
		} elseif ( $days > 0 ) {
			return __( 'a day', 'block-gallery' );
		}
		...
		if ( $hours > 1 ) {
			...
			return sprintf( __( '%s hours', 'block-gallery' ), $hours );
		} elseif ( $hours > 0 ) {
			return __( 'an hour', 'block-gallery' );
		}
		...
		if ( $minutes > 1 ) {
			...
			return sprintf( __( '%s minutes', 'block-gallery' ), $minutes );
		} elseif ( $minutes > 0 ) {
			return __( 'a minute', 'block-gallery' );
		}
		...
		if ( $seconds > 1 ) {
			...
			return sprintf( __( '%s seconds', 'block-gallery' ), $seconds );
		} elseif ( $seconds > 0 ) {
			return __( 'a second', 'block-gallery' );
		}
	}
		...
		<div class="notice updated blockgallery-notice">
			<div class="blockgallery-notice-inner">
				<div class="blockgallery-notice-icon">
					...
					<img src="https://ps.w.org/block-gallery/assets/icon-256x256.jpg" alt="<?php printf( esc_attr__( '%1$s WordPress Plugin', 'block-gallery' ), esc_attr( $this->name ) ); ?>" />
				</div>
				<div class="blockgallery-notice-content">
					<?php /* translators: 1. Name */ ?>
					<h3><?php printf( esc_html__( 'Are you enjoying %1$s?', 'block-gallery' ), esc_html( $this->name ) ); ?></h3>
					<p>
						<?php /* translators: 1. Name, 2. Time */ ?>
						<?php printf( esc_html__( 'You have been using %1$s for %2$s now! Mind leaving a quick review to let us know know what you think? I\'d really appreciate it!', 'block-gallery' ), esc_html( $this->name ), esc_html( $time ) ); ?>
					</p>
				</div>
				<div class="blockgallery-install-now">
					<?php printf( '<a href="%1$s" class="button button-primary blockgallery-install-button" target="_blank">%2$s</a>', esc_url( BLOCKGALLERY_REVIEW_URL ), esc_html__( 'Leave a Review', 'block-gallery' ) ); ?>
					<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No thanks / I already have', 'block-gallery' ); ?></a>
				</div>
			</div>
		</div>
		<?php
